...
@section('content')
	<a href="{{ route('backend.pages.create') }}" class="btn btn-primary">Create a New Page</a>
	<table class="table table-hover">
		...
		<tbody>
			@if($pages->isEmpty())
				<tr>
					<td colspan="5" align="center">There are no pages to display.</td>
				</tr>
			@else
				...
				@foreach($pages as $page)
					<tr class="{{ $page->hidden ? 'warning' : '' }}">
						...
						<td>
							<a href="{{ url($page->uri) }}">{{ $page->pretty_uri }}</a>
						</td>
